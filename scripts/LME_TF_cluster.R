...
tic('reading files..')
for (n in 1:nsamples) {
  file1 <-read.csv(paste('../mydata/TFold/sample_',n+59,'.csv',sep=''),header=FALSE)
...
for (f in 100:120) {
  ...
  write.csv(out_orig,file = paste('../mydata/TFold/output/orig_TF_freq',f,'.csv',sep = ''))
  toc()
}
#permutations
for (f in 100:120) {
  ...
  write.csv(out_perm,file = paste('../mydata/TFold/output/perm_TF_freq',f,'.csv',sep = ''))
}
stopCluster(cl)