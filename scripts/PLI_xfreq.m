...
for subj = 1:length(data)
   ...
   %comb_ch = allcomb(data(subj).chanHwm,data(subj).chanHwm);
    %load bad trials
    ...
    for comb = 1:length(comb_ch)
        ...
        for tri = 1:size(D_chan,3)
            ...
            [b, a] = butter(2, [1, 4]./(spm_data.fsample/2)); %cutoff: 1 to 8 Hz
            Dnew = filtfilt(b, a, D_chan(comb_ch(comb),:,tri)); %amygdala channels
            complex_ref(1, :) = hilbert(Dnew);
            %second source
            [c, d] = butter(2, [30, 80]./(spm_data.fsample/2)); %cutoff: 1 to 8 Hz
            Dtemp = filtfilt(c, d, D_chan(comb_ch(comb,2),:,tri)); %hippocampus channels
...
%         all_sub_clust{1,subj} = sub_clust;
    ...
    for c = 1:length(comb_ch)
        ...
        %names{1,c} = ['ch_' num2str(comb_ch(c))];
    end
...
writetable(all_PLI,'PLI_del_lg_ALHC.csv');
